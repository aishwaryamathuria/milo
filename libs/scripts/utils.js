...
export function lazyLoadLottiePlayer($block = null) {
  ...
  if (lottie !== 'off') {
    const loadLottiePlayer = () => {
      ...
      script.async = true;
      script.src = '/libs/scripts/lottie-player.1.5.6.js';
      document.head.appendChild(script);
...
export function getIcon(icons, alt, size = 44) {
  ...
  if (symbols.includes(icon)) {
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-${icon}">
      ${alt ? `<title>${alt}</title>` : ''}
      <use href="/libs/icons/ccx-sheet_${sheetSize}.svg#${iconName}${sheetSize}"></use>
    </svg>`;
  } else {
    return (`<img class="icon icon-${icon}" src="/libs/icons/${icon}.svg" alt="${alt || icon}">`);
  }
}
...
export async function fetchRelevantRows(path) {
  if (!window.relevantRows) {
    try {
      ...
      const resp = await fetch(`${prefix}/libs/relevant-rows.json`);
      window.relevantRows = resp.ok ? (await resp.json()).data : [];
    } catch {
      const resp = await fetch('/libs/relevant-rows.json');
      window.relevantRows = resp.ok ? (await resp.json()).data : [];
    }
  }
...
function decorateHeaderAndFooter() {
  ...
  header.addEventListener('click', (event) => {
    if (event.target.id === 'feds-topnav') {
      const root = window.location.href.split('/libs/')[0];
      window.location.href = `${root}/libs/`;
    }
  });
...
export async function loadBlock(block, eager = false) {
  if (!(block.getAttribute('data-block-status') === 'loading' || block.getAttribute('data-block-status') === 'loaded')) {
    ...
    let cssPath = `/libs/blocks/${blockName}/${blockName}.css`;
    let jsPath = `/libs/blocks/${blockName}/${blockName}.js`;
    ...
    if (window.hlx.experiment && window.hlx.experiment.run) {
      ...
      if (experiment.selectedVariant !== 'control') {
        ...
        if (control && control.blocks && control.blocks.includes(blockName)) {
          ...
          cssPath = `/libs/experiments/${experiment.id}/${blockPath}/${blockName}.css`;
          jsPath = `/libs/experiments/${experiment.id}/${blockPath}/${blockName}.js`;
        }
      }
    }
...
export async function fetchPlaceholders() {
  ...
  if (!window.placeholders) {
    try {
      const { prefix } = getConfig().locale;
      await requestPlaceholders(`${prefix}/libs/placeholders.json`);
    } catch {
      await requestPlaceholders('/libs/placeholders.json');
    }
  }
...
function loadMartech() {
  ...
  const analyticsUrl = '/libs/scripts/instrument.js';
...
function loadGnav() {
  ...
  const gnavUrl = '/libs/scripts/gnav.js';
...
export async function getExperimentConfig(experimentId) {
  ...
  if (instantExperiment) {
    ...
    const path = `/libs/experiments/${experimentId}/manifest.json`;
    try {
      const config = {};
...
async function decorateTesting() {
  try {
    ...
    if ((checkTesting() && (martech !== 'off') && (martech !== 'delay')) || martech === 'rush') {
      ...
      loadScript('/libs/scripts/instrument.js', 'module');
    }
  } catch (e) {
    // eslint-disable-next-line no-console
...
export async function fetchFloatingCta(path) {
  ...
  if (['yes', 'true', 'on'].includes(dev) && env && env.name === 'stage') {
    spreadsheet = '/libs/floating-cta-dev.json?limit=100000';
  } else {
    spreadsheet = '/libs/floating-cta.json?limit=100000';
  }

  if (experimentStatus === 'active' || experimentParams) {
    const expSheet = '/libs/experiments/floating-cta-experiments.json?limit=100000';
...
async function buildAutoBlocks($main) {
  ...
  if (['yes', 'true', 'on'].includes(getMetadata('show-relevant-rows').toLowerCase())) {
    ...
    if (!authoredRRFound && !window.relevantRowsLoaded) {
      ...
      if (relevantRowsData) {
        ...
        const fragment = buildBlock('fragment', '/libs/fragments/relevant-rows-default-v2');
  ...
  async function loadPromoFrag() {
    const fragment = await fetchPlainBlockFromFragment('/libs/fragments/rejected-beta-promo-bar', 'sticky-promo-bar');
    if (!fragment) return;
...
async function loadLazy(main) {
  addPromotion();
  loadStyle('/libs/styles/lazy-styles.css');
  scrollToHash();
  resolveFragments();
  removeMetadata();
  addFavIcon('/libs/icons/cc-express.svg');
  sampleRUM('lazy');
...
export async function loadArea(area = document) {
  ...
  if (getMetadata('sheet-powered') === 'Y' || window.location.href.includes('/libs/templates/')) {
